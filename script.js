...
// ── POLYFILL: roundRect (Safari < 15.4, eski Edge) ───────────────────────────
...
setupModels().catch(err => {
  ...
  modelStatus.textContent = "● MODEL HATASI";
...
function createPersonCard(slotIdx) {
  ...
  card.innerHTML = `
    ...
    <div class="person-card-body">
      ...
      <!-- YÜZ: Baş Açıları (sadece yüz algılandığında görünür) -->
...
// ── YÜZ → KİŞİ EŞLEŞTİRME ───────────────────────────────────────────────────
// Her algılanan yüzü, omuz noktasına (lm[0] = burun ucu) en yakın pose slotuna atar.
// Sadece burun ucu ile pose'un baş noktası (lm[0] = burun) arasındaki mesafeye bakılır.
function matchFacesToSlots(faceLandmarks, poseLandmarks, poseToSlot) {
  // faceToSlot[i] = i. yüzün atandığı slot indeksi (-1 ise eşleşme yok)
  const faceToSlot = new Array(faceLandmarks.length).fill(-1);
  const slotUsed   = new Set();

  for (let fi = 0; fi < faceLandmarks.length; fi++) {
    const faceNose = faceLandmarks[fi][1]; // burun ucu
    if (!faceNose) continue;

    let minDist = 0.3; // eşik — bu mesafeden uzak pose'a bağlanma
    let bestSlot = -1;
...
async function predictWebcam() {
  ...
  if (lastVideoTime !== video.currentTime) {
    ...
    // ── 3 Model Aynı Frame'de Çalıştır ────────────────────────────────────
    ...
    // ── Yüz → Slot Eşleştirme ─────────────────────────────────────────────
    // Her aktif slot için yüz verisi tutuyoruz; frame başında sıfırla
    const slotFaceLm = {};   // slotIdx → faceLandmarks
    const slotFaceBs = {};   // slotIdx → faceBlendshapes
    ...
    if (faceLandmarks.length > 0 && poseLandmarks.length > 0) {
      ...
      for (let fi = 0; fi < faceLandmarks.length; fi++) {
        ...
        if (slot !== -1) {
          ...
          slotFaceBs[slot] = faceBlendshapes[fi] ?? null;
        }
      }
    }
    ...
    for (let i = 0; i < poseCount; i++) {
      ...
      // Kişi etiketi (omuzlar ortası üstünde)
      ...
      // ── Bu kişiye ait yüz çizimi + metrikleri ─────────────────────────
      const faceLm = slotFaceLm[slotIdx];
      const faceBs = slotFaceBs[slotIdx];

      if (faceLm) {
        // Yüz oval + dudak + göz konturları (hafif, sadeleştirilmiş)
        canvasCtx.save();
        canvasCtx.globalAlpha = 0.85;
        drawingUtils.drawConnectors(faceLm, FaceLandmarker.FACE_LANDMARKS_FACE_OVAL, {
          color: color.hex, lineWidth: 1.5
        });
        drawingUtils.drawConnectors(faceLm, FaceLandmarker.FACE_LANDMARKS_LIPS, {
          color: color.hex, lineWidth: 1.2
        });
        drawingUtils.drawConnectors(faceLm, FaceLandmarker.FACE_LANDMARKS_LEFT_EYE, {
          color: "#30FF30", lineWidth: 1.2
        });
        drawingUtils.drawConnectors(faceLm, FaceLandmarker.FACE_LANDMARKS_RIGHT_EYE, {
          color: "#FF3030", lineWidth: 1.2
        });
        canvasCtx.globalAlpha = 1;
        canvasCtx.restore();
      }

      // Panel güncelle (yüz yoksa bölümü gizler)
